        ...
        <div className="overflow-hidden rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-slate-900 shadow-sm transition-colors duration-300">
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-950/50 text-slate-500 dark:text-gray-400 uppercase tracking-widest text-[10px] font-bold whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                        {data.map((employee, index) => (
                            ...
                            >
                                <td className="px-6 py-5 text-slate-400 dark:text-gray-500 font-medium whitespace-nowrap">{String(index + 1).padStart(2, '0')}</td>
                                <td className="px-6 py-5 font-bold text-slate-900 dark:text-gray-200 group-hover:text-[#13624C] dark:group-hover:text-emerald-400 transition-colors whitespace-nowrap">
                                    {employee.nama}
                                </td>
                                ...
                                <td className="px-6 py-5 font-semibold text-slate-700 dark:text-gray-300 whitespace-nowrap">{employee.jabatan}</td>
                                <td className="px-6 py-5 text-[#13624C] dark:text-emerald-400 font-bold whitespace-nowrap">Rp {employee.gaji?.toLocaleString('id-ID')}</td>
                                <td className="px-6 py-5 text-right flex justify-end gap-2 whitespace-nowrap min-w-[150px]">
                                    {onEdit && (
